  ...
  return echapper(texte)
    ...
    .replace(
      ...
      (_, libelle, url) =>
        ...
        // L'URL est aussi échappée avant insertion dans l'attribut — la
        // regex qui l'a capturée exclut déjà espaces et parenthèses, mais on
        // ne veut pas que la sécurité tienne à ce détail d'implémentation.
        /^(https?:\/\/|\/|#)/i.test(url)
          ? `<a href="${echapper(url)}">${libelle}</a>`
          : libelle,
    );
}
